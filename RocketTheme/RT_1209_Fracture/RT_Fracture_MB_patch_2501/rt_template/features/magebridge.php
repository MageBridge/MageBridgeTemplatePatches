...
class GantryFeatureMageBridge extends GantryFeature {
	...
	function init() {
		...
		if($mb->isLoaded()) {		
			...
			//$gantry->addStyle($gantry->templateUrl.'/css/com_magebridge/magebridge-'.$mainstyle.".css");
			// Style Inclusion
			//$colorstyle = $gantry->get('colorstyle');
			//$gantry->addStyle($gantry->templateUrl.'/css/com_magebridge/color-'.$colorstyle.'.css');
