...
class GantryFeatureMageBridge extends GantryFeature {
	...
	function init() {
		...
		if($mb->isLoaded()) {		
	        ...
	        $css = null;
			$css .= '.magebridge-module button.button,#magebridge-content button.button {background-color:'.$gantry->get('body-accent').';}'."\n";
	        $css .= '.magebridge-module button.button span,#magebridge-content button.button span{background: transparent url("'.$path.'/overlays/'.$bodystyle.'/readon-bg.png")};'."\n";
